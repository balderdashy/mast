...
Mast.Tree = {
	...
	initialize: function (attributes,options){
		...
		this.collection = new (Mast.mixins.provisionPrototype(this.collection,Mast.models,Mast.Collection))();
	...
	renderBranches: function (silent,changes) {
		...
		this.$branchOutlet = (this.branchOutlet) ? Mast.mixins.verifyOutlet(this.branchOutlet,this.$el) : this.$el;
	...
	appendBranch: function (model,options,silent) {
		...
		if (options && !_.isUndefined(options.at)) {
			var $outlet = Mast.mixins.verifyOutlet(this.outlet, r.parent && r.parent.$el);
			r.render();
