...
_.extend(Framework.Component.prototype, {
	...
	_renderRegions: function () {
      ...
      if (Mast.prototyping) {
        var branchComponent = $(el).attr('template');
        var count = $(el).attr('count');

        if ((count && !branchComponent) || (branchComponent && !count)) {
          Framework.error('you must define both `count` and `component` attributes you\'d like to use the list prototyping feature.');
        } else {
          while (count >= 0) {
            region.append(branchComponent);
            count--;
          }
        }
      }
