...
Framework.Component.prototype.close = function ( ) {
	...
	this.beforeClose(function () {
		...
		// Unbind model and/or collection
		self.model && self.model.off();
		self.collection && self.collection.off();
...
_.extend(Framework.Component.prototype, {
	...
	initialize: function (properties) {
		...
		// Iterate through each key on this component
		_.each(this, function (value, key) {

			// Detect subscriptions and bind an event listener
			if (key.match(/^(%|#|~)/)) {
				if (!_.isFunction(value)) {
					throw new Error(this.id + ':: Invalid listener for subscription: ' + key);
				}
				Mast.on(key, value);
			}
		});
