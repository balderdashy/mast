(function(){
	...
	pressFoo(27,'gPressEscape',true);
	pressFoo(13,'gPressEnter',true);
	...
	function pressFoo(keyCode,handlerName,globalScope) {
		var pressEvent = function (origTarget,keyCode,handlerName) {
			return function(e) {
				...
				{
					e.preventDefault();
					origTarget.triggerHandler( handlerName );
				}
			}
		}
		...
		$.event.special[handlerName] = {
			...
			setup: function( eventData, namespaces ){
				...
				if (origTarget.data(dataKeyName)) {
					target.unbind("keyup",eventHandler);
					origTarget.data(dataKeyName,false);
				}
				target.bind("keyup", eventHandler);
			...
			teardown: function( namespaces ) {
				...
				// Bind the event handler that we are going
				// to need to make sure this event fires correctly.
				target.unbind( "keyup", origTarget.data(dataKeyName));
				origTarget.data(dataKeyName,false);
